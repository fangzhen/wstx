...
public class MsgContextUtil {
	...
	public static <T extends AbstractCoordinatorProtocolService> T getTargetCoorService(
		...
		if (coorService == null) {
			String errMsg = "Failed to get corresponding coordinator protocol service with id: " + id;
			if (__LOG.isErrorEnabled()) {
				__LOG.error(errMsg);
			}
			throw new WstxRtException(errMsg);
		}
		return coorService;
	}
	/**
	...
	public static <T extends AbstractParticipantProtocolService> T  getTargetPtcpService(
		...
		if (ptcpService == null) {
			String errMsg = "Failed to get corresponding participant protocol service with id: " + id;
			if (__LOG.isErrorEnabled()) {
				__LOG.error(errMsg);
			}
			throw new WstxRtException(errMsg);
